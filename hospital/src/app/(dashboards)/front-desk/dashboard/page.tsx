...
export default function FrontDeskDashboard() {
    ...
    useEffect(() => {
        ...
        fetch("/api/patients")
            ...
            .then(data => {
                if (data.patients) {
                    setCheckInQueue(data.patients);
                }
            })
            .catch(err => console.error(err));
    }, [showInTake]); // Refresh when toggling back from intake
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {checkInQueue.length === 0 ? (
                                            <tr><td colSpan={5} className="px-8 py-8 text-center text-slate-400 font-bold text-xs">No active patients found.</td></tr>
                                            ...
                                            <tr key={idx} className="hover:bg-slate-50/50 transition-colors group">
                                                ...
                                                <td className="px-8 py-5">
                                                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase border ${p.status === 'Active' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                                        {p.status}
                                                    </span>
                                                </td>
                                                <td className="px-8 py-5 text-xs font-bold text-slate-700">{p.condition}</td>
