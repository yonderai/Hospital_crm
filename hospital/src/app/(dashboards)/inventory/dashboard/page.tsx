...
export default function InventoryDashboard() {
    const stats = [
        { title: "Total SKU Count", value: "1,240", icon: Box, color: "text-olive-600", bg: "bg-olive-50" },
        { title: "Critical Stock", value: "18", icon: AlertTriangle, color: "text-red-500", bg: "bg-red-50" },
        { title: "Active Orders", value: "05", icon: Truck, color: "text-blue-500", bg: "bg-blue-50" },
        { title: "Monthly Spend", value: "$42k", icon: BarChart3, color: "text-olive-500", bg: "bg-olive-50" },
    ];
        ...
        <DashboardLayout>
            <div className="space-y-10">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Supply Chain & Logistics</h2>
                        <p className="text-slate-500 text-sm font-medium mt-1 uppercase tracking-widest">LOGS DIR. JEFF BEZOS • ASSET MANAGEMENT</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="lg:col-span-2 bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
                        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                            <h3 className="text-xl font-black text-slate-900 tracking-tight">Inventory Stock Ledger</h3>
                            <button className="text-[10px] font-black text-olive-600 uppercase tracking-[0.2em]">Procurement Plan</button>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                                        <th className="px-8 py-4">Item Name / Category</th>
                                        <th className="px-8 py-4">Stock Level</th>
                                        <th className="px-8 py-4">Status</th>
                                        <th className="px-8 py-4">Usage Trend</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {inventoryItems.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase tracking-tight">{item.trend}</td>
                            ...
                            <div className="relative z-10 space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-4 p-4 bg-olive-500/10 rounded-2xl border border-olive-500/20">
                                        <Package className="text-olive-400" size={20} />
                                        <div>
                                            <p className="text-sm font-bold text-olive-100">Asset Audit Stage 2</p>
                                            <p className="text-[10px] text-olive-300 mt-0.5 uppercase tracking-widest font-black">98% Verified</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
