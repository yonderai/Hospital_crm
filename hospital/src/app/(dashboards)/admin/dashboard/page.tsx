...
export default function AdminDashboard() {
    const [stats, setStats] = useState([
        ...
        { title: "Total Patients", value: "--", icon: TrendingUp, color: "text-olive-600", bg: "bg-olive-50" },
        { title: "Revenue (Est)", value: "$--", icon: DollarSign, color: "text-green-500", bg: "bg-green-50" },
        { title: "Pending Appts", value: "--", icon: AlertCircle, color: "text-red-500", bg: "bg-red-50" },
    ]);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                if (data.stats) {
                    setStats([
                        { title: "Total Staff", value: data.stats.totalStaff.toString(), icon: Users, color: "text-blue-500", bg: "bg-blue-50" },
                        { title: "Total Patients", value: data.stats.totalPatients.toString(), icon: TrendingUp, color: "text-olive-600", bg: "bg-olive-50" }, // Changed label for clarity
                        { title: "Revenue (Est)", value: `$${data.stats.monthlyRevenue.toLocaleString()}`, icon: DollarSign, color: "text-green-500", bg: "bg-green-50" },
                        { title: "Pending Appts", value: data.stats.pendingApprovals.toString(), icon: AlertCircle, color: "text-red-500", bg: "bg-red-50" },
                    ]);
                }
            } catch (e) {
                console.error(e);
