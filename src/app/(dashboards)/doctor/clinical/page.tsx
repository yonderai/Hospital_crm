...
function DoctorClinicalContent() {
    const searchParams = useSearchParams();
    const [tasks, setTasks] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedPatient, setSelectedPatient] = useState<any>(null);
    const [initialTab, setInitialTab] = useState<string | undefined>(undefined);
    const [aptId, setAptId] = useState<string | undefined>(undefined);
    ...
    const fetchClinicalFeed = async () => {
        try {
            ...
            const [labsRes, rxRes, consultRes, apptsRes, surgeryRes] = await Promise.all([
            ...
            if (consultRes.ok) {
                ...
                const pendingConsults = consults
                    .filter((c: any) => c.status !== 'closed')
                    .map((c: any) => ({
                        id: c._id,
                        title: `Resume Consultation: ${c.chiefComplaint}`,
                        patient: `${c.patientId?.firstName} ${c.patientId?.lastName}`,
                ...
                combinedTasks = [...combinedTasks, ...pendingConsults];
            }

            if (apptsRes.ok) {
                const appts = await apptsRes.json();
                const pendingAppts = appts
                    .filter((a: any) => !['completed', 'cancelled', 'no-show'].includes(a.status))
                    .map((a: any) => ({
                        id: a._id,
                        title: `Start Consultation: ${a.reason}`,
                        patient: `${a.patientId?.firstName} ${a.patientId?.lastName}`,
                        patientId: a.patientId?._id,
                        appointmentId: a._id, // Use _id for internal linking
                        type: "Appointment",
                        priority: "High",
                ...
                combinedTasks = [...combinedTasks, ...pendingAppts];
            }

            if (surgeryRes.ok) {
                const surgeries = await surgeryRes.json();
                const pendingSurgeries = surgeries
                    .filter((s: any) => s.status === 'scheduled' || s.status === 'in-progress')
                    .map((s: any) => ({
                        id: s._id,
                        title: `Surgical Procedure: ${s.procedureName}`,
                        patient: `${s.patientId?.firstName} ${s.patientId?.lastName}`,
                        patientId: s.patientId?._id,
                        type: "Surgery",
                        priority: "Urgent",
                        time: s.startTime || new Date(s.scheduledDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                        rawDate: new Date(s.scheduledDate)
                    }));
                combinedTasks = [...combinedTasks, ...pendingSurgeries];
            }
                                        ...
                                        tasks.map((task, idx) => (
                                            <div key={idx} className="p-8 flex items-center justify-between group hover:bg-slate-50 transition-colors">
                                                <div className="flex items-center gap-6">
                                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border shadow-inner ${task.type === 'Appointment' ? 'bg-orange-50 text-orange-600 border-orange-100' : task.priority === 'Urgent' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-olive-50 text-olive-600 border-olive-100'
                                                        }`}>
                                                        {task.type === 'Laboratory' ? <Beaker size={24} /> : (task.type === 'Consultation' || task.type === 'Appointment') ? <Stethoscope size={24} /> : <Pill size={24} />}
                                                    </div>
                                                    <div>
                                                ...
                                                <button
                                                    onClick={() => {
                                                        const fetchPatient = async () => {
                                                            const res = await fetch(`/api/patients/${task.patientId}`);
                                                            if (res.ok) {
                                                                const data = await res.json();
                                                                setSelectedPatient(data);
                                                                setInitialTab("consultation");
                                                                setAptId(task.appointmentId);
                                                                setEncounterId(task.isAppt ? undefined : task.id);
                                                            }
                                                        };
                                                        fetchPatient();
                                                    }}
...
export default function DoctorClinical() {
    return (
        <Suspense fallback={
            <DashboardLayout>
                <div className="p-20 text-center text-slate-300 font-black uppercase animate-pulse tracking-[0.3em]">
                    Synchronizing Clinical Streams...
                </div>
            </DashboardLayout>
        }>
            <DoctorClinicalContent />
        </Suspense>
    );
}
