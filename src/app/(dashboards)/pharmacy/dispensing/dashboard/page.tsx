                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                        {prescriptions.length === 0 ? (
                            ...
                            >
                                ...
                                <h4 className="font-bold text-slate-800">{rx.patientId.firstName} {rx.patientId.lastName}</h4>
                                <p className="text-xs text-slate-400 mt-1">{rx.medications.length} items • Dr. {rx.providerId.lastName}</p>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="h-full flex flex-col">
                            <div className="flex justify-between items-start mb-8 pb-8 border-b border-slate-100">
                                <div>
                                    ...
                                    <p className="text-slate-500 text-sm mt-1 ml-9">MRN: {selectedRx.patientId.mrn}</p>
                                </div>
                                <div className="text-right">
                                    <div className="text-xs font-bold text-slate-400 uppercase">Prescribed By</div>
                                    <div className="font-medium text-slate-700">Dr. {selectedRx.providerId.firstName} {selectedRx.providerId.lastName}</div>
                            ...
                            <div className="pt-6 border-t border-slate-100 mt-auto">
                                <div className="flex items-center justify-between bg-teal-50 p-4 rounded-xl mb-6">
                                    ...
                                    <span className="text-sm font-bold text-teal-900">Charges will be auto-added to Patient Invoice</span>
                                </div>
